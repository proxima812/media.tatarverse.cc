...
main.config.ts                 значения (единственный файл под правку)
  ...
  └─ src/config/validate.ts    fail-fast на старте dev/build
       │
       ├─→ astro.config.mjs    передаёт конфиг интеграциям ЯВНО, аргументами
...
Всё, что видно в браузере, приходит из двух мест: `Layout.astro` (HTML) и
...
| `astro.config.mjs` | Сборка интеграций. Читает `config`, раздаёт аргументы. Нативный i18n-роутинг, `@astrojs/sitemap` с i18n, `dualmark` (markdown-двойники, `llms.txt`). |
...
2. **Никакого module-level mutable state.** Всё живёт в замыкании фабрики.
...
Рендер вынесен в чистую функцию — её можно вызвать и проверить без сборки.

## Build-time и runtime

- `astro.config.mjs`, `src/config/validate.ts`, `src/integrations/**` — **только Node**,
  выполняются на сборке. `node:fs` здесь разрешён.
...
Коллекций сейчас нет намеренно — пустая заглушка была удалена.
Если контент нужен, создать `src/content.config.ts` с **настоящим loader'ом**:
...
Коллекция без `loader` — это legacy-режим. Не создавать её «на будущее»:
пустая коллекция выглядит как рабочий код и вводит в заблуждение.

## Astro 7

- Node >= 22.12.
- `output: "static"`. Адаптер не подключён — стартер разворачивается как статика.
