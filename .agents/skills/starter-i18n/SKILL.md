...
`routing.prefixDefaultLocale: false` — `defaultLocale` живёт без префикса
(`/`, `/about`), остальные локали — с префиксом (`/en/`, `/en/about`). Так же
устроен `tatarverse`. Дефолтная локаль остаётся на «родных» URL, если i18n
добавляют в уже существующий проект — маршруты не переезжают.

Страница `src/pages/404.astro` **не** локализуется: статический хостинг отдаёт
...
это не `tatarverse` с частичным покрытием переводов: здесь каждая локаль
обязана покрывать все маршруты, где она подключена.
...
2. `src/i18n/locales/<code>.ts` — новый словарь с теми же ключами, что у
   ...
   `dictionaries` — забытый файл упадёт на `useTranslations` тихим фолбэком на
...
Также добавить локаль в `localeDicts` в `astro.config.mjs`, если у неё есть
