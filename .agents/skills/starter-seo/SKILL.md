...
Вся мета-разметка живет в **одном** месте: `src/components/SEO/SEO.astro`.
...
- **JSON-LD** - через `schema-dts`, типизированно. `type="article"` дает
...
Теперь это `@dualmark/astro`, подключенный в `astro.config.mjs` - он же генерирует
...
- `dualmarkStaticPages` в `astro.config.mjs` - по одной записи `{ pattern, render }`
  ...
  `astro.config.mjs` прошел через Vite, и обычные замыкания/`import()` там ломают
...
- Появится первая content collection - замапить ее в `dualmark({ collections })`
  (см. `starter-architecture` про Content Layer) - тогда ее записи попадут
...
(`middleware.injectLinkHeader: false`), а alternate-ссылка на markdown отдается
