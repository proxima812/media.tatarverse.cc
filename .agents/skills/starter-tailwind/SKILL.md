...
Пакет `@astrojs/tailwind` устарел (он для v3) и удален - не возвращать.
...
- **Не** ставить `text-primary` на body только потому, что «получилось темное».
...
- Не плодить классы-обертки вида `.btn` - использовать компонент `.astro`.
