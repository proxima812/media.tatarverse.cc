...
татарские и не только узоры) и всё, что рассказывает об этих народах.
...
источник истины. Перевод живёт отдельно, в `src/data/cards-en/<id>.md`
...
- `cards-i18n-baseline.json` — список карточек без перевода, заведённых до
...
bun run verify      # всё сразу (lint + check + check:seo + check:i18n)
```
