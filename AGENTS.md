...
Крупная правка - больше трех файлов, новая фича, изменение схемы карточек
или таксономии - начинается с плана, а не с кода. План лежит в
`docs/superpowers/plans/<YYYY-MM-DD>-<тема>.md`, шаблон -
`docs/superpowers/plans/TEMPLATE.md`. Если решение неочевидно и его нужно
обсудить отдельно от порядка работ, спека кладется рядом в
`docs/superpowers/specs/<YYYY-MM-DD>-<тема>-design.md` и указывается в плане.

Один каталог планов на репозиторий - второго не заводить.

План коммитится до реализации отдельным `docs:`-коммитом. Это дает точку,
где видно намерение до того, как появилась тысяча строк диффа. Мелкие правки
- опечатка, одна карточка, подгонка стиля - плана не требуют.
