...
Восемь стадий: он открывает нужную страницу, говорит что нажать, принимает
скопированное значение и сам вписывает его в `main.config.ts`. Перед коммитом
гоняет `bun run verify` и проверяет, что теги дошли до `dist/`; коммит и пуш -
только с подтверждения. Прервать можно в любой момент: собранные значения
лежат в `.wizard-launch.env` (не коммитится), повторный запуск их предложит.

Подтверждение владения разнесено с получением кодов намеренно: мета-тег должен
быть уже на живом сайте, иначе консоль его не увидит.
