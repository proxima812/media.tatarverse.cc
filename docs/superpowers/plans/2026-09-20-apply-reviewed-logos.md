...
- [x] Применить выбор через скрипт с проверкой хешей и резервными копиями.
- [x] Проверить пути, неизменность остального контента, типы и i18n.
...
Результат: применено 147 логотипов (11 024 334 байта). Все PNG и хеши проверены, остальной текст карточек совпадает с исходным. `bun run check`: 0 ошибок и предупреждений; `bun run check:i18n`: 325/325. Резервные копии: `backup-20260920T081858478810Z/` в папке отчета.
