/**
 * Основа генератора фона: палитра, детерминированный PRNG и правка светлоты
 * по оттенку. Единственный потребитель - `meshGradient.ts`.
 *
 * Контракт генератора: `(seed: string) => string`, где результат - готовая
 * CSS-декларация (`background-color` + `background-image`). Одна и та же
 * строка на входе всегда дает один и тот же фон: карточка не должна менять
 * вид между сборками.
 *
