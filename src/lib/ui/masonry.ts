 ...
 * в первой колонке, 4 - только потом во второй), поэтому порядок чтения идет
 * вниз-потом-вбок, а не по строкам, как в обычной сетке. Раскладка по
 * колонкам-round-robin (0 → колонка 0, 1 → колонка 1, …) дает порядок как в
 * `grid-auto-flow`, при этом каждая колонка остается независимым flex-потоком
...
export class MasonryGrid {
	...
	/** Кладет карточку в следующую по кругу колонку и запоминает ее для будущего пересчета при ресайзе. */
	...
	private buildColumns(): void {
		/*
		 * Флаг для разметки: до него контейнер стоит в CSS-колонках, чтобы
		 * карточки не мелькали одной строкой до выполнения скрипта
		 * (см. комментарий в `CatalogGrid.astro`).
		 */
		this.container.dataset.ready = "";
